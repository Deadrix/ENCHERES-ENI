...
public class AuctionDetailServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		// display seller (broken temporarily : peu importe ce que je fais, eclipse demande des try catch partout. Si vous voulez test faites un cc arpès User ci-dessous : seller = articleManager.selectById(no_article).getSeller;
		User;
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (thisPickUp != null) {
			...
			if (finalPrice == 0) {
				...
				articleManager.selectById(no_article).getSeller().setCredit(articleManager.selectById(no_article).getSeller().getCredit() + buyerCredit);
			} else {
